...
// definition location.
export function getDefinitionAtPosition(buffer, path, position) {
  var result = {
    import: null,
    definition: null,
    ...
    for (let symbol of globalScope.exportedSymbols) {
      if (symbol.importLocation && positionIsInsideLocation(position, symbol.importLocation)) {
        if (symbol.moduleRequest)
          result.definition = findSymbolDefinition(symbol.importName, symbol.moduleRequest, null, false, scope);
      }
    }
  }
...
export function getReferencesAtPosition(buffer, path, position, params={}) {
