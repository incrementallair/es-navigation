...
//Jump to position in given path from editor, with option range to select.
function jumpToPositionFrom(position, path, editor, range=null, toggle=null) {
  ...
  if (path == editor.getPath()) {
    ...
    if (range) editor.setSelectedBufferRange(range);
    if (toggle) {
    ...
    atom.workspace.open(path, {
      ...
      if (range) openedEditor.setSelectedBufferRange(range);
      if (toggle) {
        toggle.position = previousCursor;
        toggle.path = previousPath;
      }
